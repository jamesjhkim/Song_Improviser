...
int main(void) {
  ...
  //   square_arr[i][0] = rand() % (319 - 2 * R_CIRCLE) + R_CIRCLE;
  //   square_arr[i][1] =
  ...
  *(pixel_ctrl_ptr + 1) = (int)&Buffer2;
  ...
  while (1) {
    ...
    for (int i = 0; i < 40; i++) {
      for (int j = 0; j < 40; j++) {
        if (test[j][i] != 0) {
          plot_pixel(i, j, (short)0xF81F);
          if (i % 2 == 0 && j % 2 == 0) {
            plot_pixel(100 + i / 2, 100 + j / 2, (short)0x07E0);
          }
        }
      }
    }
    ...
    //   for (int i = x_plot - R_CIRCLE; i < x_plot + R_CIRCLE; i++) {
    ...
    //   for (int x_cnt = centre_x - R_CIRCLE; x_cnt < centre_x + R_CIRCLE;
    //        x_cnt++) {
    //     for (int y_cnt = centre_y - R_CIRCLE; y_cnt < centre_y + R_CIRCLE;
    //          y_cnt++) {
    //       if ((x_cnt - centre_x) * (x_cnt - centre_x) +
    //               (y_cnt - centre_y) * (y_cnt - centre_y) <=
    //           R_CIRCLE * R_CIRCLE) {
