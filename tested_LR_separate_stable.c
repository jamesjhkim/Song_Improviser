...
int main() {
	...
	int period1_1 = 8000/261.63;
	int period1_2 = 8000/329.63;
	int period1_3 = 8000/392;
	int period1_4 = 8000/523.25;
	...
	int period2_1 = 8000/130.81;
	int period2_2 = 8000/164.81;
	...
	while (1) {
		...
		if (out_fifo_1 >= period_total_1 / 30) {
			for (int j = 0; j < period_total_1 / 30; j++) {
				*(audio_ptr + 3) = buffer_1[buffer_ptr_1] * 0xFFFF;    
				buffer_ptr_1 = (buffer_ptr_1 + 1) % BUFFER_SIZE_1;
			}
		}

		// Right speaker output (chord 2)
		if (out_fifo_2 >= period_total_2 / 30) {
			for (int j = 0; j < period_total_2 / 30; j++) {
				*(audio_ptr + 2) = buffer_2[buffer_ptr_2] * 0xFFFF;  
